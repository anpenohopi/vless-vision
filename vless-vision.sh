...
update_script() {
	fail=0
	ol_version=$(curl -sL github.com/anpenohopi/vless-vision/raw/vision/vless-vision.sh | grep "script_version=" | head -1 | awk -F '=|"' '{print $3}')
	if [[ ! $(echo -e "$ol_version\n$script_version" | sort -rV | head -n 1) == "$script_version" ]]; then
		wget -O xray-yes-en.sh github.com/anpenohopi/vless-vision/raw/vision/vless-vision.sh || fail=1
